...
export interface CoverflowSlide {
  /** Image or video URL. Omit for a typographic placeholder card. */
  src?: string;
  media?: "image" | "video";
  alt: string;
  title?: string;
...
export function CoverflowCarousel({
  ...
  const endDrag = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    // A press that barely moved is a tap: centre card activates, a side card
    // comes to the centre. Pointer capture retargets the event to the frame,
    // so hit-test the point instead of reading event.target.
    if (Math.abs(event.clientX - drag.x) < 4) {
      const hit = document.elementFromPoint(event.clientX, event.clientY);
      const card = hit?.closest<HTMLElement>("[data-cf-index]");
      const index = card ? Number(card.dataset.cfIndex) : NaN;
      const centre = indexAt(posRef.current);
      if (Number.isInteger(index) && index !== centre) {
        goTo(index);
        return;
      }
      settle(clamp(Math.round(posRef.current)));
      if (index === centre) onActivateRef.current?.(centre);
      return;
    }
    ...
    >
        ...
        <div
          ...
          onKeyDown={(event) => {
            if (event.key === "ArrowLeft") {
              ...
              event.preventDefault();
              onActivateRef.current?.(indexAt(targetRef.current));
            }
          }}
      ...
      {showCaption && active?.title && (
        ...
        >
          ...
          {active.subtitle && (
            <p className="mt-1 max-w-md text-center text-[13px] text-muted-foreground sm:text-[14px]">
              {active.subtitle}
            </p>
          )}
          {active.meta && active.meta.length > 0 && (
            <dl className="mt-8 w-full max-w-[280px] text-[12px] sm:text-[13px]">
              {active.meta.map((row) => (
